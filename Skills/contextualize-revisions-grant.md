---
description: Contextualize grant revision targets against a source application and saved writing constraints
---

You are coordinating the grant-specific contextualization stage of the post-review editing pipeline.

Your job is to turn a saved `revision-plan.json` and a saved `writing-constraints.json` for one grant application into a deterministic `contextualized-edit-plan.json` that downstream drafting can consume.
...
- maps each abstract revision target to grant-application sections or prompts
- checks the proposed localization against sponsor and application constraints
...
This skill is grant-specific. It does not implement paper, PAP, or paper-code contextualization.

## Runtime Rules

- Work from local repository files only.
- Produce exactly one contextualized edit plan for one grant source document per run.
...
- Prefer exact source headings and prompt labels when recoverable.
...
2. If `revision_plan=` is absent, search the resolved source's deterministic editing directory for `revision-plan.json`.
3. If `writing_constraints=` is absent, search the resolved source's deterministic editing directory for `writing-constraints.json`.
...
4. Confirm both artifacts have `document_type = grant`.
...
## Phase 3: Read Grant Source Structure

Read enough source-document structure to localize each revision target.

Prefer these source cues:

- numbered application questions
- Roman-numeral section headers
- prompt labels such as `Problem Statement`, `Proposed Solution`, `Expected Outcomes`, `Project Budget and Timeline`, and `Ethics and Compliance`
- visible text surrounding likely target prompts
- comments or unresolved notes only when they affect placement, not as reviewer evidence
...
- add a `context_notes` entry explaining that final field-level placement should be checked against the source form

## Phase 4: Build Grant Context Model

For grants, map each target to one or more of these location families:

- public-benefit and theory-of-change fields
- core narrative and proposed-solution fields
- outcomes, metrics, and 12-month impact fields
- feasibility, milestones, budget, and staffing fields
- responsible-AI, ethics, governance, and release fields
- duplicated or competing answer blocks across application prompts

Use the smallest credible final location, but add `propagation_targets` when the same change must be checked across multiple grant sections.
...
- `propagation_targets` when grant consistency needs cross-section checks
- `constraint_checks`
- `source_evidence`

### Location Rules

Populate `location.section` with a grant-facing section or prompt family. Use `heading_match`, `paragraph_hint`, `text_quote`, and `source_path` when recoverable.
...
- "Revise the expected-outcomes answer so the month-12 public good is named before longer-run clinical aspirations."

Not allowed:

- drafted replacement paragraphs
- invented quantitative claims
- new methods, partners, budgets, or outcomes not present in the source artifacts
...
- required sections
- style requirements
- audience and public-benefit expectations
- source-local prompt limits when recoverable

## Phase 6: Resolve Deterministic Output Path

Save under the same deterministic grant editing directory as the shared-stage artifacts:
...
3. Confirm `document_type` is `grant`.
4. Confirm `revision_plan_path` and `writing_constraints_path` point to the saved shared-stage artifacts used for this run.
5. Confirm the output path follows `artifacts/grants/<source-slug>/editing/contextualized-edit-plan.json` or the documented `-vN` rule.
...
7. any uncertainty left for the later `draft-edits-grant` stage
...
- implement `draft-edits-grant`
- support papers, PAPs, or paper-code packages
- change shared schemas to make the grant path easier
